...
function Desktop() {
  ...
  const closeSC_Manager = () => {
    toggleSC_Manager(false);
  };

  const openSC_Manager  = () => {
    toggleSC_Manager (true);
  };

  const [SC_ManagerOpened, toggleSC_Manager] = useState(false);


  /* SC_model Shortcut */

  const closeSC_Process = () => {
    toggleSC_Process(false);
  };

  const openSC_Process = () => {
    toggleSC_Process(true);
  };

  const [SC_ProcessOpened, toggleSC_Process] = useState(false);
